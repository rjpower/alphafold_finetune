...
flags.DEFINE_string(
    "train_dataset",
    None,
    help="tsv file with training dataset. See " "README for format info",
)
...
flags.DEFINE_bool(
    "test_load", False, help="if True, loop through datasets to test loading"
)
flags.DEFINE_bool(
    "dump_pdbs",
    False,
    help="if True, write out PDB files of " "modeled structures during training",
)
flags.DEFINE_integer(
    "print_steps", 25, help="printed averaged results every print_steps"
)
flags.DEFINE_integer("save_steps", 100, help="save model + optimizer every save_steps")
flags.DEFINE_integer(
    "valid_steps", 100000, help="calc loss on whole valid set every valid_steps"
)
...
flags.DEFINE_float(
    "fake_native_weight",
    0.25,
    help="weight to apply to the alphafold "
...
flags.DEFINE_float(
    "grad_norm_clip", 10.0, help="value to clip gradient norms, " "per update"
)
flags.DEFINE_string(
    "data_dir",
    "/home/pbradley/csdat/alphafold/data/",
...
flags.DEFINE_bool(
    "random_recycling",
    False,
    help="if True, set num_iter_recycling " "randomly during training",
)
flags.DEFINE_bool("plddt_binder", False, help="if True, fit model based on pLDDT")
flags.DEFINE_float(
    "pae_binder_slope",
    -7.9019634017508675,  # from logistic regression
    help="initial slope for PAE binder model",
)
flags.DEFINE_float(
    "plddt_binder_slope",
    16.690735,  # from logistic regression
...
assert (
    FLAGS.msa_clusters >= 5
)  # since reduce_msa_clusters_by_max_templates is True for model_1/model_1_ptm
    ...
    * predicted_lddt dict
    ...
    def __call__(self, input_features):
        ...
        x_intercept = hk.get_parameter(
            "x_intercept",
            shape=[plddt.shape[-1], num_binder_classes],
            dtype=plddt.dtype,
            init=x_intercept_init,
        )
...
class PaeBinderClassifier(hk.Module):
    ...
    * predicted_aligned_error dict
    ...
    def __call__(self, input_features):
        ...
        x_intercept = hk.get_parameter(
            "x_intercept",
            shape=[pae.shape[-1], num_binder_classes],
            dtype=pae.dtype,
            init=x_intercept_init,
        )
...
if not FLAGS.plddt_binder:
    ...
    assert all(
        x < 2.0 for x in FLAGS.binder_intercepts
    ...
    assert all(
        x < 1.0 for x in FLAGS.binder_intercepts
...
if not FLAGS.plddt_binder:  ## PAE model
    ...
    classifier_dict["binder_class_1hot"] = jnp.ones(
        [1, num_binder_classes], dtype=jnp.float32
    )
...
def train_step(model_params, key, batch, structure_flag):
    (loss, (predicted_dict, binder_loss, prob, features)), grads = jax.value_and_grad(
        get_loss_fn, has_aux=True
    )(model_params, key, batch, structure_flag)
...
def prep_batch_for_step(batch, training):
    ...
    batch["chi_mask"] = torsion_dict["torsion_angles_mask"][
        :, :, 3:
    ]  # [B, N, 4] for 4 chi
    ...
    if training:
        if FLAGS.random_recycling:
            batch["num_iter_recycling"] = jnp.array(
                np.tile(
                    np.random.randint(0, model_config.model.num_recycle + 1, 1)[None,],
                    (batch_size, model_config.model.num_recycle),
                ),
                jnp.int32,
            )
            ...
            print(
                "not setting num_iter_recycling!!! will do",
                model_config.model.num_recycle,
                "recycles",
            )

    # binder stuff:
    batch["binder_class_1hot"] = jnp.array(
        batch["binder_class_1hot"], dtype=jnp.float32
    )
...
def compute_valid_stats(valid_loader, replicated_params, jax_key):
    ...
    for n, batch in enumerate(valid_loader):
        ...
        print(
            ...
            loss[0],
            "lddt_ca=",
            np.mean(predicted_dict["predicted_lddt"]["lddt_ca"]),
            "fape=",
            np.mean(predicted_dict["structure_module"]["fape"]),
            "binder_probs=",
            binder_probs,
            "binder_loss=",
            binder_loss[0],
        ...
        temp_sidechain_fape.append(
            np.mean(predicted_dict["structure_module"]["sidechain_fape"])
        )
        ...
        print(
            f"test_Step: {global_step} {n}, loss: {mean_loss}, binder_loss: {mean_binder_loss} lddt: {lddt_ca}, fape: {fape}, sc_fape: {sidechain_fape}, chi: {chi_loss}, disto: {distogram}, msa: {masked_msa}",
            flush=True,
        )
        sys.stdout.flush()
...
for e in range(num_epochs):
    ...
    for n, batch in enumerate(train_loader):
        ...
        if (
            grads_sum_count >= FLAGS.apply_every  # time to update
            and not FLAGS.freeze_everything
        ):
            print("grad update!", global_step, grads_sum_count)
            grads_sum = jax.tree_map(lambda x: x / grads_sum_count, grads_sum)
            grads_sum = norm_grads_per_example(
                grads_sum, l2_norm_clip=FLAGS.grad_norm_clip
            )
        ...
        if (n + 1) % FLAGS.print_steps == 0:
            ...
            chi_loss = round(float(np.mean(temp_chi_loss)), 4)
            print(
                f"Step: {global_step}, loss: {mean_loss}, binder_loss: {binder_loss} lddt: {lddt_ca}, fape: {fape}, sc_fape: {sidechain_fape}, chi: {chi_loss}, disto: {distogram}, msa: {masked_msa}",
                flush=True,
            )
            temp_train_loss = []
            temp_binder_loss = []
